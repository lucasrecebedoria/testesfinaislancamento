...
async function gerarRelatorioPDF() {
  ...
  // Adicionando logo e cabeçalho
  const logo = new Image();
  logo.src = 'MOVE_BUSS_LOGO02.png'; // garantir que a logo está na pasta public ou root do projeto
  docpdf.addImage(logo, 'PNG', 80, 10, 50, 30); 
  y = 60;
  docpdf.setFont('helvetica','bold');
  docpdf.setFontSize(14);
  docpdf.text('RELATÓRIO DE FECHAMENTO DE CAIXA', 105, y, { align: 'center' });
  y += 20;
  docpdf.setFontSize(12);
  docpdf.text(`Matrícula: ${currentUserDoc.matricula}`, 105, y, { align: 'center' });
  y += 14;
  docpdf.text(`Nome: ${currentUserDoc.nome||''}`, 105, y, { align: 'center' });
  y += 14;
  docpdf.text(`Abertura: ${caixaData?.abertura||''}`, 105, y, { align: 'center' });
  y += 14;
  docpdf.text(`Fechamento: ${new Date().toLocaleString('pt-BR')}`, 105, y, { align: 'center' });
  y += 20;
  ...
  // Linha final para assinatura
  docpdf.setLineWidth(0.5);
  docpdf.line(40, y+20, 160, y+20);
  docpdf.text('Assinatura do Responsável', 105, y+35, { align: 'center' });
